...
class Film {
	...
	public static function getMovieDetail($currentId){
		global $pdo;
		// Je récupère le paramètre d'URL "page" de type integer
		
		$sql = '
			SELECT fil_id, fil_titre, fil_annee, fil_affiche, fil_synopsis, fil_acteurs, fil_filename, cat_id, sup_id, fil_description
			FROM film
			WHERE fil_id = :filId';
		$pdoStatement = $pdo->prepare($sql);
